...
public class ModConfig {
	...
	public static class Patch {
		...
		public static class Fix {
			...
			static {
				BUILDER.comment("Configure fixes to issues that occur when some items are made stackable.").push("fixes");
				
				LIMIT_SADDLE_SLOT = BUILDER
						.comment("Limit the capacity of the horse/mule/donkey saddle slot to 1, since equipping more than one saddle is redundant.")
						.define("limit_saddle_slot", true);
				
				LIMIT_BANNER_PATTERN_SLOT = BUILDER
						.comment("Limit the capacity of the loom's banner pattern slot to 1, since inserting more than one banner pattern is "
				...
				FIX_ANVIL_ENCHANTING = BUILDER
						.comment("When a stack of enchanted books is applied in the anvil, enchant one item at a time instead of "
								+ "consuming more ingredients, requiring fewer ingredients, or requiring more experience than necessary.")
						.define("fix_anvil_enchanting", true);
