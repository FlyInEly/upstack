...
public abstract class ThrowablePotionItemMixin {
    ...
    * using the cooldown duration (in ticks) supplied by {@link CommonConfig.Cooldowns#POTION_THROW_COOLDOWN}, if greater
    * than zero.
    *
   ...
   @Unique
	protected InteractionResultHolder<ItemStack> upstack$useWithCooldown(Level level, Player player, InteractionHand hand, Operation<InteractionResultHolder<ItemStack>> original) {
      int duration = CommonConfig.Cooldowns.POTION_THROW_COOLDOWN.getAsInt();
      if (duration > 0) {
